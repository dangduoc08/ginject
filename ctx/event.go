...
func (e *Event) On(eventName string, fn func(args ...any)) {
	...
	e.mu.Unlock()
	if max > 0 && n > max {
		fmt.Fprintln(os.Stderr, color.FmtYellow(
...
func callEventListenerSafe(l func(...any), args []any) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Fprintln(os.Stderr, color.FmtRed("EventEmitter: listener panic recovered: %v", r))
		}
	}()
	l(args...)
}
