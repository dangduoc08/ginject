...
func TestAddToEventMap_InitMaps(t *testing.T) {
	ws := &WS{}
	ws.addToEventMap("SUBSCRIBE_message", "chat_message", nil)
	if ws.EventMap == nil {
...
func TestAddToEventMap_StoresEntries(t *testing.T) {
	ws := &WS{}
	ws.addToEventMap("SUBSCRIBE_message", "chat_message", "handler")
	if ws.EventMap["chat_message"] != "handler" {
		t.Error(testutils.DiffMessage(ws.EventMap["chat_message"], "handler", "EventMap entry"))
	}
	if ws.patternToFnNameMap["chat_message"] != "SUBSCRIBE_message" {
		t.Error(testutils.DiffMessage(ws.patternToFnNameMap["chat_message"], "SUBSCRIBE_message", "patternToFnNameMap entry"))
	}
}
...
func TestAddHandlerToEventMap_Stores(t *testing.T) {
	...
	ws := &WS{}
	ws.AddHandlerToEventMap("chat", "SUBSCRIBE_message", nil)
	if ws.EventMap == nil {
	...
	if InsertedEvents[eventName] != "SUBSCRIBE_message" {
		t.Error(testutils.DiffMessage(InsertedEvents[eventName], "SUBSCRIBE_message", "InsertedEvents entry"))
	}
}
